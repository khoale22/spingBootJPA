...
<div id="container" style="vertical-align: top; overflow: auto; background-color: #D6DCE2;">
	<form:form action="/protected/cps/add/AddNewCandidate" id="addItemForm" modelAttribute="addNewCandidate">
		...
		<tr>
			...
			<fieldset style="vertical-align: top; overflow: auto; background-color: #FFFFFF">
				...
				<table border="0" width="100%" align="center">
					<tr>
						<td class="dataGridHead" class="labelFont" align="center" width="30%">Available UPC</td>
						<td class="dataGridHead" align="center" class="labelFont" width="30%">Scan Status</td>
						<c:if test="${addNewCandidate.testScanUserSwitch}">
							<td class="dataGridHead" align="center" class="labelFont" width="30%">Override Test scan</td>
						</c:if>
					</tr>
					<tbody id="values">
					<c:forEach items="${addNewCandidate.upcVOs}" var="product" varStatus="loop">
						<tr class="labelFont">
							<c:url var="img" value="${request.getContextPath()}/hebAssets/images/newButtons/validated.PNG" />
							<td align="center" width="30%" id="upcRow${loop.index}" class="labelFont">
								<font class="labelFont"><c:out value="${product.unitUpc}"></c:out></font>
								<input type="hidden" id="prodId${loop.index}" value="${product.psProdId}" />
								<input type="hidden" id="sequanceId${loop.index}" value="${product.seqNo}" />
								<input type="hidden" id="upcTyp${loop.index}" value="${product.upcType}" />
							</td>
							<td align="center" width="30%" class="labelFont">
								<c:choose>
									<c:when test="${product.testScanOverridenStatus eq 'N'.charAt(0) && product.testScanUPC ne ''}">
										<c:set value="visibility: visible;position: static;" var="styleStr" />
									</c:when>
									<c:otherwise>
										<c:set value="visibility: hidden;position: static;" var="styleStr" />
									</c:otherwise>
								</c:choose>
								<div id="validateDiv${loop.index}" style="${styleStr}">
									<img src='${img}' alt="Validated" style="position: static;" />
								</div>
							</td>
							<c:if test="${addNewCandidate.testScanUserSwitch}">
								<td align="center" width="30%" class="labelFont">
									<c:if test="${product.testScanOverridenStatus eq 'Y'.charAt(0)}">
									<input type="checkbox" id="checkDiv${loop.index}" checked="checked" onclick="clearScan();">
									</c:if>
					...
					<tr>
						...
						<td align="left" width="25%">
							<div id="holder" style="position:absolute; z-index:99; background-color:transparent">
								<form:input styleClass="textFieldNormal" path="scanGun" tabindex="73" maxlength="15" id="scanGun"
											styleId="scanGun" size="15" onkeydown="checkForValue();" readonly="true"/>
							</div>
							<div id="reader" align="left"  style="width:10px">
								<textarea onkeypress="onKeyPressEvent(event);" rows="1" cols="1"
										  style="text-decoration:none; text-align:right;  border: 0px solid #000000;
										  width:1px; cursor:none;"
										  name="scanGunReader" id="scanGunReader"></textarea>
							</div>
						</td>

						<td align="left" width="25%" class="dataGridHead" background="#FFFFFF">
							<button type="button" id="validate" name="validate" value="Add">ValidateUPC</button>
		...
		<script type="text/javascript">
            ...
            function testSacnOnLoad(){
                <c:if test="${addNewCandidate.rejectClose}">
                //var ex = "fromTestScan();";
                window.parent.fromTestScan();
                //window.parent.execScript("f2();");
                window.parent.closePopup();
                </c:if>
            }
            ...
            function callBack(data) {
                var scriptString = "testScanMess1('"+data+"');";
                window.parent.testScanMess1(data);
                //window.parent.execScript("f2();");
                window.parent.closePopup();
                window.parent.nonSellableHideNew();
            }
            ...
            function setfocusIn(){
                //YAHOO.util.Dom.get("scanGunReader").focus();
