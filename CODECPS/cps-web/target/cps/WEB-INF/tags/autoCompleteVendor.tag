...
<%@attribute  name="elmName" required="false" rtexprvalue="false" %>
<%@attribute  name="onSelectMethod" required="false" rtexprvalue="false" %>
<%@attribute  name="onSelectMethod1" required="false" rtexprvalue="false" %>
<%@attribute  name="onSelectMethod2" required="false" rtexprvalue="false" %>
...
<%@attribute  name="elmProperty" required="false" rtexprvalue="false" %>
...
<c:choose>
	...
	<c:otherwise>
		<c:set value="999" var="realMaxResults"></c:set>
	</c:otherwise>
</c:choose>
...
<c:choose>
	<c:when test="${vendorVO.formattedVendorLocation != null}">
		<c:set value='value="${vendorVO.formattedVendorLocation}"' var="formattedValueText"></c:set>
	</c:when>
	<c:when test="${selectedVendorVO.formattedVendorLocation != null}">
		<c:set value='value="${selectedVendorVO.formattedVendorLocation}"' var="formattedValueText"></c:set>
	</c:when>
	<c:otherwise>
		<c:set value="" var="formattedValueText"></c:set>
	</c:otherwise>
</c:choose>
<c:choose>
	<c:when test="${vendorVO.vendorLocation != null}">
		<c:set value='value="${vendorVO.vendorLocation}"' var="valueText"></c:set>
	</c:when>
	<c:when test="${selectedVendorVO.vendorLocation != null}">
		<c:set value='value="${selectedVendorVO.vendorLocation}"' var="valueText"></c:set>
	</c:when>
	<c:otherwise>
...
<c:choose>
	<c:when test="${vendorVO.vendorLocationVal != null}">
		<c:set value='value="${vendorVO.vendorLocationVal}"' var="valueId"></c:set>
	</c:when>
	<c:when test="${selectedVendorVO.vendorLocationVal != null}">
		<c:set value='value="${selectedVendorVO.vendorLocationVal}"' var="valueId"></c:set>
	</c:when>
	<c:otherwise>
...
<!-- START AUTOCOMPLETEVENDOR TAG. UNIQUE ID: ${uniqueId}    -->
...
<c:if test="${needToShowGenericCSS}">
	...
	<script type="text/javascript">
		YAHOO.namespace("YAHOO.HEB.autocompleteVendor");
	</script>
...
<div id="${uniqueId}AutoComplete">
    <input id="${uniqueId}ACInput" type="text" ${tabIdxStr} ${formattedValueText} /><nobr></nobr><span id="${uniqueId}Toggle"></span>
	<div id="${uniqueId}ACContainer"></div>
</div>	
				
<c:if test="${elmProperty != null}">
	<input type="hidden" id="vendorLocationVal" ${valueId}></input>
</c:if>

<c:if test="${elmName != null}">
	<input type="hidden" id="vendorLocation" ${valueText}></input>
</c:if>
...
<c:url value="/protected/cps/add/manage/modules/search/backend/ajaxSearch?action=${searchAction}&showId=${realShowId}&highlightMatch=${realHighlightMatch}&maxResults=${realMaxResults}&searchOnId=${realSearchOnId}&uniqueId=${uniqueId}&" var="jsonSearch"></c:url>
    ...
    bdmAC.maxResultsDisplayed = ${realMaxResults};
		...
		<c:if test="${elmProperty != null}">
			if(oPayload && (oPayload.query != undefined) && (oPayload.query != null)){
				if("" == oPayload.query){
					...
					var xElm = YAHOO.util.Dom.get("vendorLocationVal"); 
	...
	var itemSelectHandler = function(sType, aArgs) {
		...
		<c:if test="${elmName != null}">
			...
			YAHOO.util.Dom.get("vendorLocation").value=aData.name;		
		</c:if>
		...
		<c:if test="${onSelectMethod1 != null}">
			setTimeout(function() { // For IE
	            ${onSelectMethod1}.apply(document, aArgs);
	        },0);
		</c:if>		
		<c:if test="${onSelectMethod2 != null}">
		setTimeout(function() { // For IE
            ${onSelectMethod2}.apply(document, aArgs);
        },0);
	</c:if>	
