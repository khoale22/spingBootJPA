		...
		<table width="100%">
			<tr>
				...
				<td align="right" width="50%">
					<input type="button" id="resetId" value="Reset" onclick="resetRejectUPCComment();"></input>
					<input type="button" id="cancelId" value="Cancel" onclick="toClose();"></input>
					<input type="button" id="saveId" value="Save" onclick="saveRejectComment();"></input>
...
<%
	...
	%> <script type="text/javascript">  
		dataUPCComment = {   
  		...
  		var dataSourceTemp = [];
		var dataUPCCommentResult = makeRejectCommentTable();
		
		function makeRejectCommentTable(){
        ...
        var myColumnDefs = [{key:"changeStatus",    label:""},	
                            ...
                            {key:"sellingUPCs",    label:"Selling UPC", width: 120, minWidth: 120, sortable:true, sortOptions:{sortFunction:sortSellingUpc},
                            ...
                            {key:"descriptions",    label:"Description", width: 200, minWidth: 200, sortable:true, sortOptions:{sortFunction:sortDescription},
			 ...
			 for (var i= 0; i< this.getRecordSet().getLength();i++){
		        ...
		        if(idWorkRq == idWorkRqOfCheckBox){
		        	if(elCheckbox.checked){
						...
						oRec.setData("changeStatus", getChangeStatusOnRejectCommentPopup(oRec.getData("changeStatus"), true));
		        	}
		        	else
		        	{
		        		...
		        		oRec.setData("changeStatus", getChangeStatusOnRejectCommentPopup(oRec.getData("changeStatus"), false));
			...
			if (column.key == 'checkBox') {
				if(typeof check != 'undefined'){	
					...
					for(var i=0; i<oRS.getLength(); i++) {
						...
						{
							oRec.setData("checkBox","<input type=checkbox id=ckb_"+ oRec.getData("idResult") +" name=ckb_"+ oRec.getData("idResult").split('-')[0] +" checked=checked class=yui-dt-checkbox />");		
							var changeValue = getChangeStatusOnRejectCommentPopup(oRec.getData("changeStatus"), true);
	    	            	if(changeValue != null)
						...
						{
						 	oRec.setData("checkBox","<input type=checkbox id=ckb_"+ oRec.getData("idResult") +" name=ckb_"+ oRec.getData("idResult").split('-')[0] +" class=yui-dt-checkbox />");
						 	var changeValue = getChangeStatusOnRejectCommentPopup(oRec.getData("changeStatus"), false);
	    	            	if(changeValue != null)
	           					oRec.setData("changeStatus", changeValue);
						}  
					}
	...
	function getChangeStatusOnRejectCommentPopup(oldValue, checked)
	{
		...
		if(isCheckAll){
			for(var i=0; i<dataSourceTemp.length; i++) {
	            if(checked)
	            {
	            	...
	            	var changeValue = getChangeStatusOnRejectCommentPopup(dataSourceTemp[i].changeStatus, true);
	            	if(changeValue != null)
       			...
       			else
       			{
       				...
       				var changeValue = getChangeStatusOnRejectCommentPopup(dataSourceTemp[i].changeStatus, false);
	            	if(changeValue != null)
	            		dataSourceTemp[i].changeStatus = changeValue;
       			}    	               
			}
    ...
    {	
    	...
    	var myDataTable = dataUPCCommentResult.oDT;
...
function toClose(evt){
	if(confirm("Exit Reject Comment? (click OK to exit)")){
...
function resetRejectUPCComment()
{
	var myDataSource = dataUPCCommentResult.oDS;
	var myDataTable = dataUPCCommentResult.oDT;
...
function saveRejectComment(){
	if(dataUPCComment != null){
		...
		var currentPage = dataUPCCommentResult.oDT.get('paginator').getState().page;
		var currentRows = dataUPCCommentResult.oDT.get('paginator').getState().rowsPerPage;
		...
		{   	
			...
			if(arrIdResult.length >0)
			{
			    ...
			    ManageEDIDWR.saveRejectComment(arrIdResult,arrRejectReasonsSelected,arrSuggestedActionsSelected, 
			   	{
			    	callback:function(data) {
				    	window.parent.hideTheProgress();
				    	var dataOld = dataUPCComment.areacodes;
				    	var dataChange = eval(data.appData);
						//update datasource
	    				if(dataChange.length >0){	 	    				
		    				for(var i=0; i< dataOld.length; i++){
		 	    				for(var j=0; j< dataChange.length; j++){
	 	 	    					if(dataOld[i].idResult == dataChange[j].idResult){
			 	 	    				if(dataChange[j].rejectReason != "")
			 	 	    					dataOld[i].rejectReason = dataChange[j].rejectReason;
			 	 	    				if(dataChange[j].suggestedAction != "")
			 	 	    					dataOld[i].suggestedAction = dataChange[j].suggestedAction;
		 	 	    					break;
	 	 	    					}
		 	    				}
		    				}
	 	    				...
	 	    				//reset data source temp
	 	    					dataSourceTemp = [];
	 	    					dataUPCCommentResult = makeRejectCommentTable();
	 	    					if(dataChange.length == 1 || dataChange.length > 1){
	 	    						YAHOO.util.Dom.get('saveResult').innerHTML ="<span style='color: green;'><b>Reject Comment UPC(s) Updated Successfully</b></span>";
	 	    						changeRequest = false;
		    					}
		    					else
		    					{
		    						YAHOO.util.Dom.get('saveResult').innerHTML ="<span style='color: red;'><b>Reject Comment UPC(s) Updated Fails</b></span>";
		    					}
	 	    					//set current page is selected (before click reset button)
	 	    					dataUPCCommentResult.oDT.get('paginator').setRowsPerPage(parseInt(currentRows));
	 	    					dataUPCCommentResult.oDT.get('paginator').setPage(parseInt(currentPage));
		    			}
	    			},
			    ...
			    ManageEDIDWR.saveRejectComment(arrIdResult,arrRejectReasonsSelected,arrSuggestedActionsSelected, 
			   	{
			    	callback:function(data) {
	    				...
	    				if(dataChange.length >0){	 	    				
	    					...
	    					{
	    						window.parent.closePopup(false);
	    					}
		    			}
	    			},
			    	errorHandler:function(errorString, exception) {
...
<script type="text/javascript">
	if(dataUPCComment != null && dataUPCComment.areacodes.length >0){
		document.getElementById('vendorDetailsFieldSet').style.display='';
		document.getElementById('saveExitId').style.display='';
    	...
    	document.getElementById('massFillId').style.display='none';
		document.getElementById('saveExitId').style.display='none';
