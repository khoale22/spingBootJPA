/*
 * batchUploadByNutritionAndIngredientsController.js
 *
...
/**
 * Controller to support the page that allows users to download or upload excel template for Nutrition And Ingredients.
 *
 * @author vn70529
 * @since 2.12
 */
(function () {
	angular.module('productMaintenanceUiApp').controller('BatchUploadByNutritionAndIngredientsController', batchUploadByNutritionAndIngredientsController);
	batchUploadByNutritionAndIngredientsController.$inject = ['$scope', 'urlBase', 'BatchUploadResultsFactory',  'BatchUploadApi'];
	/**
	 * Constructs for batch Upload By Nutrition And Ingredients controller.
	 *
	 ...
	 */
	function batchUploadByNutritionAndIngredientsController($scope, urlBase, batchUploadResultsFactory, batchUploadApi) {
		 ...
		 * @type {string}
		 */
		self.downloadTemplateURL = urlBase + '/pm/nutritionAndIngredients/downloadTemplate/';
		/**
		...
		self.doUpload = function (event, data) {
			...
			if (!$scope.isWaitingForResponse) {
				...
				batchUploadApi.uploadNutritionIngredient(
					formData,
