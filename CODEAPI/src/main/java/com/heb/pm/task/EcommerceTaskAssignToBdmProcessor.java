/*
 * EcommerceTaskAssignToBdmProcessor
 *
...
/**
 * EcommerceTaskAssignToBdmProcessor works through every CandidateWorkRequest sent by reader. It basically does the preparation for
 * writing to the data store.
 *
 * @author vn70529
 * @since 2.17.0
 */
public class EcommerceTaskAssignToBdmProcessor implements ItemProcessor<CandidateWorkRequest, CandidateWorkRequest> {
    ...
    public CandidateWorkRequest process(CandidateWorkRequest candidateWorkRequest) {
        if (candidateWorkRequest != null) {
            ...
            if (productMaster != null && productMaster.getClassCommodity() != null) {
                Bdm bdm = bdmRepository.findOne(productMaster.getBdmCode());
                if (bdm != null && StringUtils.isNotBlank(bdm.getUserId())) {
                    // Update candidateWorkRequest
                    // recipient id of bdm = bdm.getUserId()
                    candidateWorkRequest.setLastUpdateUserId(bdm.getUserId().trim().toUpperCase());
