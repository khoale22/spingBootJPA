/*
 *  FulfillmentChannelWorkRequestCreator
 *  Copyright (c) 2017 HEB
 *  All rights reserved.
 *
 *  This software is the confidential and proprietary information of HEB.
 */
package com.heb.pm.massUpdate.job.workRequestCreators;

import com.heb.pm.entity.CandidateFulfillmentChannel;
import com.heb.pm.entity.CandidateFulfillmentChannelKey;
import com.heb.pm.entity.CandidateStatus;
import com.heb.pm.entity.CandidateStatusKey;
import com.heb.pm.entity.CandidateWorkRequest;
import com.heb.pm.entity.ProductFullfilmentChanel;
import com.heb.pm.massUpdate.MassUpdateParameters;
import org.springframework.stereotype.Service;

import java.time.LocalDateTime;
import java.util.ArrayList;

/**
 * This is the work request create for the mass update for a fulfillment channel.
 *
 * @author l730832
 * @since 2.17.0
 */
@Service
public class FulfillmentChannelWorkRequestCreator implements WorkRequestCreator {

	private static final String SETUP_STATUS_CODE = "111";

	/**
	 * This is the work request creator to alter
	 * @param productId The product ID the request is for.
	 * @param transactionId The transaction ID being used to group all the requests together.
	 * @param parameters The parameters the user wants to set the different values to.
	 * @param sourceSystem The ID of this system.
	 * @return
	 */
	@Override
	public CandidateWorkRequest createWorkRequest(Long productId, Long transactionId, MassUpdateParameters parameters, int sourceSystem) {
		CandidateWorkRequest candidateWorkRequest = WorkRequestCreatorUtils.getEmptyWorkRequest(productId,
				parameters.getUserId(), transactionId, sourceSystem);
		candidateWorkRequest.setIntent(CandidateWorkRequest.INTNT_ID_DEFAULT);
		candidateWorkRequest.setProductId(productId);
		this.setCandidateStatus(candidateWorkRequest);
		this.setFulFillmentChannel(candidateWorkRequest, parameters);

		return candidateWorkRequest;
	}

	/**
	 * Adds a candidate status to the mass update.
	 * @param workRequest the candidate work request.
	 */
	private void setCandidateStatus(CandidateWorkRequest workRequest) {
		workRequest.setCandidateStatuses(new ArrayList<CandidateStatus>());
		CandidateStatus candidateStatus = new CandidateStatus();
		CandidateStatusKey candidateStatusKey = new CandidateStatusKey();
		candidateStatusKey.setStatus(SETUP_STATUS_CODE);
		candidateStatusKey.setLastUpdateDate(LocalDateTime.now());
		candidateStatus.setKey(candidateStatusKey);
		candidateStatus.setUpdateUserId(workRequest.getUserId());
		candidateStatus.setCandidateWorkRequest(workRequest);
		candidateStatus.setStatusChangeReason(CandidateStatus.STAT_CHG_RSN_ID_WRKG);
		workRequest.setCandidateStatuses(new ArrayList<CandidateStatus>());
		workRequest.getCandidateStatuses().add(candidateStatus);
	}

	/**
	 * This sets all of the fulfillment channels.
	 * @param workRequest
	 * @param parameters
	 */
	private void setFulFillmentChannel(CandidateWorkRequest workRequest, MassUpdateParameters parameters) {
		workRequest.setCandidateFulfillmentChannels(new ArrayList<>());
		for (ProductFullfilmentChanel productFullfilmentChanel : parameters.getProductFullfilmentChanels()) {
			CandidateFulfillmentChannel candidateFulfillmentChannel = new CandidateFulfillmentChannel();
			CandidateFulfillmentChannelKey key = new CandidateFulfillmentChannelKey();
			key.setCandidateProductId(workRequest.getProductId());
			key.setFulfillmentChannelCode(productFullfilmentChanel.getKey().getFullfillmentChanelCode());
			key.setSalesChannelCode(productFullfilmentChanel.getKey().getSalesChanelCode());
			key.setWorkRequestId(workRequest.getWorkRequestId());
			candidateFulfillmentChannel.setKey(key);
			candidateFulfillmentChannel.setCandidateWorkRequest(workRequest);
			candidateFulfillmentChannel.setEffectiveDate(productFullfilmentChanel.getEffectDate());
			candidateFulfillmentChannel.setExpirationDate(productFullfilmentChanel.getExpirationDate());
			candidateFulfillmentChannel.setNewData(productFullfilmentChanel.getActionCode());
			candidateFulfillmentChannel.setCandidateWorkRequest(workRequest);
			workRequest.getCandidateFulfillmentChannels().add(candidateFulfillmentChannel);
		}
	}
}
