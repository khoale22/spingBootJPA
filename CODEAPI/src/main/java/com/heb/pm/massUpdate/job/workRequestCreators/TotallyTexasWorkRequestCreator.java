...
/**
 * Creates CandidateWorkRequests that will set a product's Totally Texas status.
 *
 * @author d116773
 * @since 2.13.0
 */
@Service
public class TotallyTexasWorkRequestCreator implements WorkRequestCreator {

	/**
	 * Creates a CandidateWorkRequest set set totally Texas either on or off. Set the boolean value
	...
	public CandidateWorkRequest createWorkRequest(Long productId, Long transactionId, MassUpdateParameters parameters,
		...
		WorkRequestCreatorUtils.addMarketingClaim(candidateWorkRequest.getCandidateProductMaster().get(0), null,
				flag, MarketingClaim.Codes.TOTALLY_TEXAS.getCode(), null, null);
