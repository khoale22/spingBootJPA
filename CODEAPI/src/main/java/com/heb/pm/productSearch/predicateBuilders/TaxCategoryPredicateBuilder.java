 ...
 */
@Service
public class TaxCategoryPredicateBuilder implements PredicateBuilder {
	...
	public ExistsClause<GoodsProduct> buildPredicate(CriteriaBuilder criteriaBuilder,
		...
		CustomSearchEntry taxCategoryEntry = null;
		for (CustomSearchEntry c : productSearchCriteria.getCustomSearchEntries()) {
			if (c.getType() == CustomSearchEntry.TAX_CATEGORY) {
				taxCategoryEntry = c;
			}
		}
		if (taxCategoryEntry == null || taxCategoryEntry.getTaxCategory() == null) {
			return null;
		}
		...
		criteria[1] = criteriaBuilder.equal(gpRoot.get(GoodsProduct_.vertexTaxCategoryCode),
				taxCategoryEntry.getTaxCategory().getDvrCode());
