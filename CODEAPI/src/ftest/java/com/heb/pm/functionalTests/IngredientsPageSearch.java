package com.heb.pm.functionalTests;

import com.heb.pm.entity.Ingredient;
import com.heb.pm.scaleManagement.IngredientService;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.test.context.ActiveProfiles;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;
import org.springframework.test.context.support.AnnotationConfigContextLoader;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by m594201 on 11/3/2016.
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(loader = AnnotationConfigContextLoader.class, classes = com.heb.pm.ApplicationConfiguration.class)
@ActiveProfiles("functional-test")
public class IngredientsPageSearch {

    /**
     * The Ingredient service.
     */
    @Autowired
    IngredientService ingredientService;

    /**
     * Test search for ingredients.
     */
    @Test
    public void testSearchForIngredients(){
        List<Long> ingredientCode = new ArrayList<>();

        ingredientCode.add(1988L);
        Pageable request = new PageRequest(1, 100, Ingredient.getDefaultSort());

    }
}
